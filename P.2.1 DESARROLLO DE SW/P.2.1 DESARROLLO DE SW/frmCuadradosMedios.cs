...
namespace P._2._1_DESARROLLO_DE_SW
{
    public partial class frmCuadradosMedios : Form
    {
        ...
        private void Generar(double digitosE, double total, double inicialE)//aqui esta el metodo al que le enviamos los datos del post pasado
        {
        ...
        private void r(double[] numeros, int digitos)
        {
            string num;
            string x;
        ...
        private void impresion(string[] xi, string[] ri, string[] ceros)
        {
            for (int i = 1; i < ri.Length; i++)
            {
                int rowEscribir = dgvDatos.Rows.Count;

                dgvDatos.Rows.Add(i, xi[i-1], cuadrado[i - 1].ToString(), ceros[i-1], xi[i].ToString(), ri[i].ToString());
            }
        ...
        private double[] recopilar(double[] numeros, int digitos)
        {
            string num;
            string x;
            string[] resultado = new string[numeros.Length];
        ...
        {
            frmDescripcion frm = new frmDescripcion(1,lblTitulo.Text);
            frm.ShowDialog();
        }
    }
}
