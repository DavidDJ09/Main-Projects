...
{
    public partial class frmMulConstante : Form
    {
        public frmMulConstante()
        {
            InitializeComponent();
        ...
        public string[] completo;   //Arreglo con los numeros completos si les falta un 0
        ...
        private void btnGenerar_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvDatos.Rows.Clear();
                Generar(4, Convert.ToDouble(nudCantidad.Value), Convert.ToDouble(txtSemilla.Text), Convert.ToDouble(txtConstante.Text));
                r(numeros, 4);
                habilitarPruebas();
            }
        }

        public void Generar(double digitosE, double total, double inicialE, double constante)//aqui esta el metodo al que le enviamos los datos del post pasado
        {
        ...
        public void impresion(string[] xi, string[] ri, string[] ceros)
        {
            for (int i = 1; i < ri.Length; i++)
            {
                ...
                dgvDatos.Rows.Add(i, xi[i-1], producto[i - 1].ToString(), ceros[i-1], xi[i].ToString(), ri[i].ToString());
            }
        ...
        {
            if (txtConstante.Text.Trim().Length>3 && txtConstante.Text.Trim().Length>0
                && Regex.IsMatch(txtConstante.Text.Trim(), "^[1-9]{1}[0-9]{3,10}$"))
            {
                erpError.Clear();
                txtConstante.BackColor=Color.LightGreen;
            }
            else
            {
                erpError.SetError(txtConstante, "Ingrese un valor numérico positivo de más de 3 dígitos");
                txtConstante.BackColor=Color.White;
            }
        }
        ...
        {
            frmDescripcion frm = new frmDescripcion(2, lblTitulo.Text);
            frm.ShowDialog();
        }
    }
}
